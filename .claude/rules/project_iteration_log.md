...
### 改动内容
1. **根因**：v5.2.120 的排序以四桶为外层，同桶内类型交错，用户期望先按类型分组（待办→会议→办文）
2. **修复**：重构 sortItems() 为两层结构：
   - 外层：按类型分3组（todo→meeting→document），与日视图 renderColumn 分列一致
   - 内层：每组内独立排序（会议用四桶+手动+领导优先级；非会议用四桶+priority+order+时间）
3. **代码审查**：APPROVE (0C/0H/2M/1L)
4. **语法检查**：通过

### 提交记录
- `bbe29c6` fix: 周/月视图排序改为先按类型分组(待办→会议→办文)再桶排序(v5.2.122)

### 验证清单
- [ ] 周视图：同一天内最上面是待办，然后会议，然后办文
- [ ] 月视图：同上
- [ ] 每个类型组内置顶在前、沉底在后、已完成在最后
- [ ] 会议按领导优先级排序正确
- [ ] 手动拖拽排序在周/月视图保留
- [ ] 版本号 v5.2.122
...
## 2026-05-28 v5.2.121 Supabase数据库迁移——带宽超限二次触发

### 改动内容
1. **背景**：旧项目 ejeiuqcmkznfbglvbkbe 第二次超出5.5GB/月免费额度，办公面板无法同步
2. **迁移**：新建 Supabase 项目 pfomqdegassaqxdyyweo，执行完整建表SQL（user_data + RLS 4策略 + updated_at触发器）
3. **代码更新**：sync.js URL + anon key 指向新项目，e2e测试文件和镜像副本同步更新
4. **用户处理**：auth不迁移，所有用户需重新注册
5. **代码审查**：code-reviewer APPROVE（纯配置变更，旧引用全清除）
6. **语法检查**：3个文件全部通过

### 提交记录
- `0519c7f` chore: 迁移Supabase数据库到新项目——带宽超限二次触发(v5.2.121)

### 验证清单
- [x] 新Supabase建表SQL执行成功（4条RLS策略确认）
- [x] 旧项目引用全清除（grep确认0匹配）
- [ ] 用户重新注册并登录
- [ ] 数据正常同步到新数据库

---

## 2026-05-28 v5.2.120 周/月视图排序与日视图不一致修复

### 改动内容
1. **根因**：calendar.js 的 `sortItems()` 使用简单排序（completed→type→time），缺少日视图的四桶分组系统（pinned/sunk）、手动order、priority权重
2. **修复**：重写 `sortItems()` 完整复刻日视图排序逻辑：
   - 四桶分组：pinned(0) → normal(1) → sunk(2) → completed(3)
   - 会议桶内：手动排序项 + 领导优先级排序项 合并
   - 非会议桶内：pinned → priority权重 → 手动order → createdAt
   - 桶内pinned子排序（code-reviewer HIGH修复）
3. **代码审查**：2 HIGH（裸app引用→getDashboardApp()+null保护、缺pinned子排序→补上）
4. **语法检查**：通过

### 提交记录
- `01d31b6` fix: 周/月视图排序与日视图不一致——复刻四桶排序系统(v5.2.120)

### 验证清单
- [ ] 周视图事项排序与日视图一致（置顶在前、沉底在后、已完成在最后）
- [ ] 月视图事项排序与日视图一致
- [ ] 会议按领导优先级排序正确
- [ ] 手动拖拽排序的顺序在周/月视图保留
- [ ] 版本号 v5.2.120
...
### 改动内容
1. **首屏脚本精简**：index.html 中 defer 脚本从18个减到4个（utils/db/app-date-view/app），其余14个改为骨架屏移除后动态加载
2. **init() 两阶段拆分**：阶段1渲染主内容→移除骨架屏→启动时钟；阶段2后台加载非首屏模块→mixin→初始化剩余功能
3. **容错加固**：try-finally 确保骨架屏始终移除、懒加载始终触发；所有 mixin 调用加 typeof 守卫 + 可选链防崩溃
4. **审查修复**：initHeaderWeather 移到阶段2、openTool 加可选链、updateCountdownNotice 加可选链、bindSyncEvents 加可选链

### 当前状态
- ✅ Code review: APPROVE（1C已修复 → 0C/0H）
- ✅ 安全审查: 无安全问题
- ✅ 本地E2E: 5/5全通过，零控制台错误
- ✅ 已提交推送 `716770b`

### 提交记录
- `716770b` perf(P1): 非首屏模块延迟加载——首屏JS从713KB降至308KB(v5.2.111)
...
### 改动内容
1. **17个同步script改为defer**：不阻塞HTML解析，defer保证按文档顺序执行（DOMContentLoaded前完成）
2. **添加4个CDN预连接**：esm.sh/skypack/unpkg/cdnjs，提前建立DNS+TLS连接

### 当前状态
- ✅ Code review: APPROVE
- ✅ E2E验证: 7项全通过（加载/控制台/主面板/天气/添加按钮/日期视图/登录）
- ✅ 已提交推送 `8b5f00a`

### 提交记录
- `8b5f00a` perf: 将核心脚本改为defer加载+添加CDN预连接，优化首次加载速度
...
## 2026-05-15 v5.2.108 编辑已完成待办不再触发提醒

### 改动内容
1. **修复编辑已完成待办自动弹提醒**：保存待办时原代码硬编码 `item.completed = false`，编辑已完成的待办会重置为未完成并清除 `reminderDismissedAt`，导致提醒重新弹出。现在保留原有 `completed` 和 `completedAt`，仅未完成事项才清除提醒标记。

### 当前状态
- ✅ Code review: APPROVE（0C/0H/1M-info）
- ✅ 已提交推送 `ef38626`
- ✅ 版本 tag `v5.2.108`

### 提交记录
- `ef38626` fix: 编辑已完成待办事项时不再重置完成状态和触发提醒

### 验证清单
- [ ] 创建带截止时间的待办 → 标记完成 → 编辑标题 → 保存后仍为已完成且不弹提醒
- [ ] 创建待办 → 编辑 → 保存后正常（未完成状态不受影响）
- [ ] 创建新待办 → 默认为未完成

---

## 2026-05-14 v5.2.106 便签多页翻页+通讯录离线可用

### 改动内容
1. **便签多页翻页**：宽不变(190px)，高 1.5 倍(285px)，JSON 多页存储 `{pages:[], currentPage:0}`
2. **翻页导航**：◀ 上一页 / 1/2 指示器 / ▶ 下一页 / + 新增空白页，右键删除当前页（>1页时）
3. **数据兼容**：`_loadStickyData()` 检测首字符 `{` 判断新旧格式，旧纯文本自动包装为单页
4. **sync 适配**：新增 `_stickyCurrentContent()` 解析 JSON 提取当前页内容，6 处 DOM 恢复改用此方法
5. **sync 数据保护**：6 处恢复后刷新 `dashboard._stickyData` 防云端多页数据被本地单页覆盖
6. **通讯录离线可用**：移除 `loadContacts()` 的 `isLoggedIn()` 清空逻辑，本地缓存始终可用

### 当前状态
- ✅ 语法检查通过（app.js + sync.js + contacts.js）
- ✅ Code review: 1H+1M+1L 已修复
- ✅ 已提交推送 `09d4cef`

### 提交记录
- `09d4cef` feat: 便签多页翻页+通讯录离线可用(v5.2.106)
...
## 2026-05-14 v5.2.105 提醒UI初始状态修复+便签优化

### 改动内容
1. **修复提醒编辑显示错误+跨日提醒不触发（根因）**：`showAddModal()` 无条件显示相对提醒 UI，截止为空时用户填写相对字段被 `saveItem()` 因 `hasDeadline=false` 丢弃，改为根据截止时间决定初始 UI
2. **日期卡片变正方形**：`.board-date-label` 加 `min-height: 190px` + `justify-content: center`
3. **便签字体放大**：`.sticky-note-lines` 14px → 16px
4. **深色便签提亮**：6 种底色均匀提亮（黄#3e3a28→#5a5440 等），横线色/文字色同步提亮
5. **便签横线对齐**：`repeating-linear-gradient` 从 27-28px 调整为 31-32px，匹配 16px×2 行高
6. **code-reviewer 修复**：scriptVersions 版本号同步更新

### 当前状态
- ✅ 语法检查通过
- ✅ Code review: 1H+1M 已修复
- ✅ 已提交推送 `1b9a8f5`

### 提交记录
- `1b9a8f5` fix: 提醒UI初始状态错误导致绝对提醒字段被忽略+便签优化(v5.2.105)

### 验证清单
- [ ] 新建待办截止时间为空 → 显示「单次/每天 在 [日期] [时间] 提醒」
- [ ] 填写截止时间 → 自动切换为「截止前N分钟」
- [ ] 清空截止时间 → 切回绝对提醒
- [ ] 绝对提醒"单次+日期+10:00"保存后再编辑 → 仍显示 10:00
- [ ] 绝对提醒到点通知栏闪烁
- [ ] 两个正方形卡片均为 190×190px 整齐排列
- [ ] 便签字体清晰可读（16px 楷体）
- [ ] 深色模式便签底色明显提亮
- [ ] 便签横线对齐文字行底部
...
## 2026-05-14 v5.2.102 截止时间默认带入+便签卡片

### 改动内容
1. **截止时间默认带入**：`showAddModal` 新建待办截止时间自动填入查看日期的年月日（`datetime-local` 格式 `YYYY-MM-DDTHH:00`），用户可清空或修改
2. **便签卡片**：日期标签卡片下方新增 170px 正方形便签卡片（黄色横线纸样式），📌 标题 + `contenteditable` 可编辑区域，用于记录周/月任务
3. **便签自动保存+云端同步**：800ms 防抖自动保存到 SafeStorage，sync.js 全部 7 个同步路径补 `office_sticky_note`
4. **JS 动态定位**：`_repositionStickyNote()` 根据日期卡片的实际高度动态计算便签 top 位置（16px 间距）
5. **修复**：code-reviewer 指出的 CSS 语法错误 + syncFromCloud 遗漏 stickyNote + scriptVersions 版本号不一致 + 焦点检测防覆盖

### 当前状态
- ✅ 语法检查通过
- ✅ Code review CRITICAL/HIGH/MEDIUM 全部修复
- ✅ 已提交推送 `a66a641`

### 提交记录
- `a66a641` fix: 便签卡片重设计——正方形便签+JS动态定位(v5.2.102)
- `e4f43e7` feat: 截止时间默认带入+便签卡片(v5.2.102)
- `05ea6a5` fix: PDF OCR人名识别Fix-E (v5.2.101)（另一会话）

### 验证清单
- [ ] 新建待办→截止时间自动带入当天日期
- [ ] 清空截止时间→切换到绝对提醒 UI
- [ ] 日视图左侧两个正方形卡片整齐排列（日期卡片+便签卡片）
- [ ] 便签卡片可编辑，刷新后内容保留
- [ ] 便签内容跨设备同步
- [ ] 移动端/平板两个卡片正常显示
- [ ] 暗色模式便签卡片样式正常
...
### 改动内容
1. **截止时间选填**：新建待办不再自动填入当前日期时间，留空可选填
2. **双模式提醒**：
   - 有截止时间：相对提醒"截止前 [N分钟] 提醒"，默认3分钟，≥1天可指定时刻
   - 无截止时间：绝对提醒"[单次/每天] 在 [日期] [时间] 提醒"
3. **新增字段**：`reminderMode`(once/daily)、`reminderDate`、`reminderTimeAbs`，有/无截止时间互斥存储
4. **通知栏文案**：绝对提醒显示"[标题] [提醒]" + "今天 14:30"或"5月20日 14:30"
5. **复制兼容**：无截止时间待办复制时不再强制加截止时间，绝对提醒的日期更新到目标日期
6. **修复发现的问题**：
   - HIGH: 编辑无提醒旧事项不再自动填入提醒默认值（editItem 空值回填）
   - MEDIUM: 新建待办未动提醒表单 → `reminderManuallySet=false`
   - MEDIUM: `reminderDate` 空时存 null 而非空字符串

### 当前状态
- ✅ 语法检查通过
- ✅ Code review: 1H+2M+2L → H/M 已修复
- ✅ 已提交推送 `dbfbe5c`

### 提交记录
- `dbfbe5c` feat: 待办提醒系统重构——截止时间选填+无截止时间绝对提醒 (v5.2.96)

### 验证清单
- [ ] 新建待办截止时间为空，显示绝对提醒 UI
- [ ] 填写截止时间后自动切换为相对提醒
- [ ] "每天"模式隐藏日期选择器
- [ ] ≥1天相对提醒显示时刻输入
- [ ] 绝对提醒到点通知栏闪烁
...
## 2026-05-13 v5.2.95 复制事项提醒闹钟属性与原事项完全一致

### 改动内容
1. **清除已关闭提醒标记**：`_contextCopyTo` 新增 `delete clone.reminderDismissedAt`，跨日期复制不应保留原日期的提醒关闭状态
2. **显式设置提醒标记**：原事项的 `reminderManuallySet` 和 `deadlineManuallySet` 均非真值时，副本显式设为 `false`，避免 undefined 引发默认提醒

### 当前状态
- ✅ 语法检查通过
- ✅ Code review APPROVED（0C / 0H / 0M / 0L）
- ✅ 已提交推送 `f67160a`

### 提交记录
- `f67160a` fix: 复制事项提醒闹钟属性与原事项完全一致 (v5.2.95)

### 验证清单
- [ ] 复制有提醒配置的事项 → 副本有相同提醒
- [ ] 复制无提醒配置的事项 → 副本无默认提醒
- [ ] 复制已关闭提醒的事项 → 副本不继承关闭状态
- [ ] 控制台无 JS 错误

---

## 2026-05-13 v5.2.94 方形卡片居中+与待办列顶部平齐

### 改动内容
1. **卡片居中**：boardView padding-left 180px，卡片170px在gutter居中（左右各5px间隙）
2. **顶部平齐**：卡片 top 改为 var(--spacing-3)=12px，与待办列 column-header 顶部对齐

### 提交记录
- `4a27d65` fix: 方形卡片居中+与待办列顶部平齐 (v5.2.94)

---

## 2026-05-13 v5.2.93 方形卡片170px+三列微右移+会议3行

### 改动内容
1. **卡片微扩+三列右移**：卡片160→170px，boardView padding-left 185px
2. **会议3行**：-webkit-line-clamp:2→3

### 提交记录
- `701c25c` fix: 方形卡片170px+三列微右移+会议3行显示 (v5.2.93)
