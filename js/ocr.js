...
2. **参会人员精确定位**：attendees只填一个人的名字，即第2列的姓名/处室名。不要填分组标签（局领导/处室）！
3. **分组标签排除**：看到”局领导””处室”这两个词时，它们只是分组标签，不是参会人。直接跳过，不要放进任何字段。
4. **合并单元格继承**：如果一行没有显式的参会人名字但有日期+会议内容，继承上方最近一次出现的参会人。
5. **地点提取**：第4列的内容直接提取到location字段。第4列主要包含会议地点（建筑物名、会议室名、酒店名等），如”市行政中心6号楼二楼电视电话会议厅”。
6. **地点识别要点**：地点通常是”XX会议室””XX厅””XX酒店””XX中心XX楼””市行政中心X号楼””市分会场”等形式。提取时保留完整路径（如”市行政中心6号楼二楼电视电话会议厅”）。
7. **跨日期会议**：如”3月25日下午-26日”必须同时填date和endDate。
8. **同名会议不同参会人**：同一会议主题出现在不同行（如钱局一行、综合处一行），每行单独输出一条！
9. **会议标题标准化**：去掉日期、时间、星期前缀和动作前缀（参加/出席/列席/组织召开等），只保留核心会议名称。
