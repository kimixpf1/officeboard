...
class OCRManager {
    ...
    async parseWithAI(text, source = '') {
        ...
        const { today, todayStr, weekDay } = this.getTodayContext();
...
4. 只返回JSON，不要解释`;
    ...
    getTodayContext() {
        ...
        return { today, todayStr, weekDay };
    }

    /**
     * 解析AI返回的JSON响应，提取并清洗items
     * @param {string} content - AI返回的原始文本
     * @param {boolean} mergeResults - 是否合并结果（默认true）
     * @returns {Array|null} 解析后的items数组，失败返回null
     */
    parseAIJsonResponse(content, mergeResults = true) {
        const jsonMatch = content.match(/\{[\s\S]*\}/);
        if (!jsonMatch) return null;
        const result = safeJsonParse(jsonMatch[0], null);
        if (!result || !result.items || !Array.isArray(result.items)) return null;
        const items = result.items
            .map(item => this.validateAndCleanItem(item))
            .filter(item => item !== null);
        if (mergeResults) {
            const merged = this.mergeRecognizedItems(items);
            return this.secondaryMergeRecognizedItems(merged);
        }
        return items;
    }
     ...
     * @param {boolean} extended - 是否使用扩展词表（包含"培训"、"学习"等）
     * @returns {string} 去除常见词后的标题
     */
    extractTitleKeywords(title, extended = false) {
        const pattern = extended
            ? /会议|研究|工作|座谈|讨论|调研|培训|学习|协调|推进|落实/g
            : /会议|研究|工作|座谈|讨论|调研/g;
    ...
    // getTestCases() 和 runBuiltInTests() 已拆分到 js/core/ocr-tests.js，通过 Object.assign 混入
    ...
    checkDuplicateInBatch(newItem, batchItems) {
        ...
        for (const existing of batchItems) {
            ...
            if (newItem.type === 'meeting') {
                ...
                // 提取标题关键词（去掉"会议"、"研究"等常见词）
                const newKeywords = this.extractTitleKeywords(newTitle);
                const existKeywords = this.extractTitleKeywords(existTitle);
    ...
    checkDuplicateItem(newItem, existingItems) {
        ...
        // 提取关键词函数
        const newKeywords = this.extractTitleKeywords(newTitle, true);
        ...
        for (const existing of existingItems) {
            if (newItem.type === 'meeting') {
                ...
                const existTitle = (existing.title || '').trim().toLowerCase();
                const existKeywords = this.extractTitleKeywords(existTitle, true);
    ...
    isSameMeetingForMerge(newItemData, existingItem) {
        ...
        const bothHaveExplicitEndDate = !!newItemData.endDate && newItemData.endDate !== newItemData.date && !!existingItem.endDate && existingItem.endDate !== existingItem.date;
        const dateMatch = newDate === existingDate && (newEndDate === existingEndDate || !bothHaveExplicitEndDate);
    ...
    // PDF解析方法已拆分到 js/core/pdf-parser.js，通过 mixin 注入
    ...
    async recognizeImageWithKimi(file, progressCallback = null, retryCount = 0) {
        ...
        try {
            ...
            const { todayStr, weekDay } = this.getTodayContext();
...
10. **括号内具体参加人优先**：当会议名称或备注中出现"（XXX参加）"这种括号内具体人名时，attendees应填写"处室名+具体人名"（如"办公室廖宣峰"），不是只写处室名，也不能写成其他领导。同时从标题中去掉括号标注。
...
- 如果会议名称或备注中有括号内的具体参加人（如”（廖宣峰参加）”），attendees填写”处室名+具体人名”（如”办公室廖宣峰”），不能写成其他领导，并从标题中去掉括号标注
`
    ...
    /**
     * 二次合并：处理同一会议因日期表述不同（半天vs跨天）导致isSameMeetingForMerge
     * 未能合并的情况。对合并后的结果再做一次宽松合并。
     */
    secondaryMergeRecognizedItems(items) {
        if (!Array.isArray(items) || items.length < 2) return items;
        // 按日期排序：start date升序、endDate降序（宽范围优先），避免顺序依赖
        const sorted = [...items].sort((a, b) => {
            const aDate = this.normalizeMeetingField(a?.data?.date || '');
            const bDate = this.normalizeMeetingField(b?.data?.date || '');
            if (aDate !== bDate) return aDate.localeCompare(bDate);
            const aEnd = this.normalizeMeetingField(a?.data?.endDate || a?.data?.date || '');
            const bEnd = this.normalizeMeetingField(b?.data?.endDate || b?.data?.date || '');
            return bEnd.localeCompare(aEnd); // 宽范围优先
        });
        const result = [];
        for (const item of sorted) {
            ...
            const newCoreTitle = this.getMeetingCoreTitle(item.data.title);
            const newDate = this.normalizeMeetingField(item.data.date);
            const newEndDate = this.normalizeMeetingField(item.data.endDate || item.data.date);
            const existingItem = result.find(candidate => {
                if (candidate.type !== 'meeting') return false;
                const existCoreTitle = this.getMeetingCoreTitle(candidate.data.title);
                if (!this.isSimilarMeetingTitle(newCoreTitle, existCoreTitle)) return false;
                const existDate = this.normalizeMeetingField(candidate.data.date);
                const existEndDate = this.normalizeMeetingField(candidate.data.endDate || candidate.data.date);
                // 宽松日期匹配：start相同，或日期范围有重叠
                const startMatch = newDate === existDate;
                const overlapCheck = newDate <= existEndDate && existDate <= newEndDate;
                if (!startMatch && !overlapCheck) return false;
                // 地点匹配（有一方为空也算通过）
                const newLoc = this.normalizeMeetingField(item.data.location);
                const existLoc = this.normalizeMeetingField(candidate.data.location);
                return !newLoc || !existLoc || newLoc === existLoc;
            });
            if (!existingItem) {
                result.push(item);
            } else {
...
【表格结构说明 — 严格按此理解】
会议安排表格为WPS生成，视觉上为4列，但输出文本中可能显示为3-4列：
- 第1列（窄）：分组标签，固定文字”局领导”或”处室”。这不是参会人！不要放进attendees！
- 第2列：姓名/处室名称。如”钱局””吴局””盛局””房局””陈局””陈主任””综合处””核算处””办公室”等。这才是attendees的值。
- 第3列：会议活动名称。通常以”日期（周X）+时间，会议名”的格式出现，例如”4月22日（周二）上午8:50，参加高新区经济大镇调研”。
- 第4列：备注/地点。包含会议地点，例如”市人大机关一楼主任会议室”。

【合并单元格 — 极其重要】
WPS表格大量使用合并单元格，文本输出中合并格的内容只出现一次：
1. “局领导””处室”标签横跨其下属所有行，只输出一次。后续行不重复出现分组标签。
2. 同一参会人（如”钱局”）如果有多个会议，姓名只出现一次，但后续每行都是一个独立会议！
3. 判断规则：如果一行有”日期+时间+会议名”，即使没有显式的姓名/分组，它仍是一个独立会议，参会人应继承上方最近一次出现的姓名。
4. **标题换行续行**：WPS表格中长标题会被自动换行拆分。拆分后的后半段标题和地点会以”续：”开头的条目单独出现（如”参会：综合处｜续：专题培训班｜地点：市司法局”）。”续：”条目必须与上方最近一条同参会人的条目合并：
   - 将”续：”后的文字追加到上一条的title末尾
   - 如果上一条没有location，使用续行的location
   - 如果续行没有参会人信息，继承上方参会人
   - 合并后只保留一条记录，不要产生两条

【核心规则 — 必须严格遵守】
1. **逐行输出**：每一行会议内容都是独立的一条记录！即使参会人名字相同，也要单独输出。
2. **参会人员精确定位**：attendees只填一个人的名字，即第2列的姓名/处室名。绝不要填分组标签（局领导/处室）！
3. **分组标签排除**：看到“局领导”“处室”这两个词时，它们只是分组标签，不是参会人。直接跳过，不要放进任何字段。
4. **合并单元格继承**：如果一行没有显式的参会人名字但有日期+会议内容，继承上方最近一次出现的参会人。但如果上方出现了新的分组标签（局领导/处室），则必须重置继承链，从新分组下重新找参会人！绝不能把"局领导"分组下的"吴局"继承到"处室"分组下的行！
5. **严禁跨行混入参会人**：每条记录的attendees只能是本行第2列的那个名字（或正确继承自上方的名字）。绝不能因为某人在前几行出现过、或者标题相似，就把他塞进不相关的记录里。每行attendees严格只属于该行！
6. **同名会议不同参会人各输出各的**：同一会议主题出现在不同行（如钱局一行、综合处一行），每行单独输出一条！系统会在后续自动合并同会议的参会人，你不需要替系统做合并。
7. **地点提取**：第4列的内容直接提取到location字段。第4列主要包含会议地点（建筑物名、会议室名、酒店名等），如“市行政中心6号楼二楼电视电话会议厅”。
8. **地点识别要点**：地点通常是“XX会议室”“XX厅”“XX酒店”“XX中心XX楼”“市行政中心X号楼”“市分会场”等形式。提取时保留完整路径（如“市行政中心6号楼二楼电视电话会议厅”）。
9. **跨日期会议**：如“3月25日下午-26日”必须同时填date和endDate。
10. **会议标题标准化**：去掉日期、时间、星期前缀和动作前缀（参加/出席/列席/组织召开等），只保留核心会议名称。
11. **括号内具体参加人优先**：当会议名称或备注中出现"（XXX参加）""（XXX、YYY参加）"这种括号内具体人名时，attendees应填写"处室名+具体人名"（如"办公室廖宣峰"），不能只写处室名（"办公室"），也不能写成别的处室或领导。如果括号内有多人，每个都加上处室名前缀。同时从标题中去掉括号标注，只保留会议核心名称。
【日期格式】
- “4月22日” → date=”2026-04-22”
- “3月25日下午-26日” → date=”2026-03-25”, endDate=”2026-03-26”
- “4月中旬” → date=”2026-04-10”（取中间日做近似）
- 跨天会议必须填写endDate！

【时间格式】
- 上午8:50 → “08:50”，下午3点 → “15:00”
- X点半 = X:30
- 时间段如”8:50-10:00” → time=”08:50”, endTime=”10:00”

【标题提取】
去掉前缀只保留核心名称：
- “4月22日（周二）上午8:50，参加高新区经济大镇调研” → title=”高新区经济大镇调研”
- “4月22日（周二）上午9:00，列席省防汛抗旱工作会议” → title=”省防汛抗旱工作会议”
...
【输出格式】严格返回JSON，不要任何解释：
{
  “items”: [{
    “type”: “meeting”,
    “data”: {
      “title”: “会议核心名称（去掉日期时间动作前缀）”,
      “date”: “YYYY-MM-DD”,
      “endDate”: “YYYY-MM-DD（跨天必填）”,
      “time”: “HH:MM”,
      “endTime”: “HH:MM（如有）”,
      “location”: “地点（从第4列备注/地点列提取）”,
      “attendees”: [“单个参会人姓名”]
    }
  }]
}
...
【待办事项识别】
识别到待办任务时：type:”todo”, priority:”high/medium/low”, deadline:”YYYY-MM-DDTHH:MM”

【办文识别】
识别到文件处理时：type:”document”, docNumber:”文号”, source:”来文单位”, progress:”pending”

【最终检查】
1. 每条记录必须是一个完整的会议/待办/办文
2. attendees数组每项只含一个人名或处室名
3. “局领导””处室”绝不出现在attendees中
4. location为第4列原文（会议地点）
5. 日期格式YYYY-MM-DD，时间格式HH:MM
6. 只返回JSON，不要任何解释
7. 每条记录的attendees仅包含本行参会人，禁止跨行混入其他行的人名或处室名
8. 如果标题中有"（XXX参加）"的括号标注，attendees填"处室名+具体人名"，标题去掉括号
9. "局领导"分组下的参会人（如吴局）绝不能出现在"处室"分组下的记录中`;
...
6. 只返回JSON，不要解释
7. 注意括号内的具体参加人（如"（廖宣峰参加）"），attendees应填写"处室名+具体人名"（如"办公室廖宣峰"），不能写成其他领导，并从标题中去掉括号标注`;
        ...
        const data = await response.json();
        const content = data.choices[0]?.message?.content || '';
        const parsed = this.parseAIJsonResponse(content, false);
        return parsed || [];
    }
    ...
    async parseWithOCRAndAI(ocrText, source = '', progressCallback = null) {
        ...
        try {
            ...
            if (mergedItems.length > 0) {
                const merged = this.mergeRecognizedItems(mergedItems);
                return this.secondaryMergeRecognizedItems(merged);
            }
...
// 注入PDF解析模块（从 js/core/pdf-parser.js 拆分）
Object.assign(OCRManager.prototype, PDFParser);

// 注入内置测试模块（从 js/core/ocr-tests.js 拆分）
Object.assign(OCRManager.prototype, OCRTests);
