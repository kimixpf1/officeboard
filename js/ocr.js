...
【核心原则 - 非常重要】
1. 一张会议通知图片 = 一个会议事项（不要拆分多个会议！）
2. 会议通知中的多项议程，统一放在agenda字段里
3. 只有完全不同的会议（不同时间/地点/参会人员）才分开识别

【会议通知识别规则 - 最重要】
会议通知通常包含以下要素，请完整提取：
1. 参会人员：通知对象、出席人员、参会人员等
2. 时间：会议日期、开始时间
3. 地点：会议室名称
4. 会议名称/主题
5. 议程：多项议题全部放在agenda字段，用换行分隔

【议程合并规则 - 关键！】
如果一张图片中包含多项议程/议题（如：议题一、议题二、议程1、议程2等）：
- 这些议程属于同一个会议！
- 不要拆分成多个会议事项！
- 将所有议程内容放在agenda字段中，用换行符分隔
- 参会人员取并集（所有议程的参会人员合并）

【议程字段格式示例】
"agenda": "议题一：汇报上半年工作总结\\n议题二：研究下半年工作计划\\n议题三：讨论预算方案"
...
【待办事项识别】
如果图片中明确是独立的待办任务（非会议议程），才识别为todo类型。

【重要提醒】
- 会议通知图片永远只返回一个meeting类型的事项！
- 多项议程合并到agenda字段！
- 不要把议程拆分成多个会议！
                    ...
                    messages: [
                        { role: 'system', content: systemPrompt },
                        {
                            ...
                            content: [
                                ...
                                {
                                    type: 'text',
                                    text: '请识别这张图片中的会议通知内容。关键要求：1.这是一张会议通知，只返回一个meeting事项；2.所有议程/议题放在agenda字段里，用换行分隔；3.完整提取参会人员、时间、地点；4.不要把多项议程拆分成多个会议！'
                                }
                            ]
                        }
                    ],
