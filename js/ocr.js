...
4. 只返回JSON，不要解释`;
    ...
    correctMeetingLocationText(value) {
        ...
        const dressCodes = /(?:[，,]\s*)?(?:着正装(?:，?西装领带)?|着便装|西装领带|浅色衬衣|打领带|不需着装)(?:[，,]?\s*)?/g;
        return this.applyCommonMeetingTextCorrections(value)
            .replace(dressCodes, '')
            .replace(/([0-9])O/g, '$10')
...
- 第4列：备注。包含地点和着装要求，例如”市人大机关一楼主任会议室，着正装”。地点和着装用逗号或空格分隔。
...
5. **地点与备注分离**：第4列的内容里，只提取实际地点（建筑物名、会议室名、酒店名等）到location字段。去掉”着正装””着便装””西装领带””浅色衬衣””打领带”等着装要求。
