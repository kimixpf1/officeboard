...
- 补上跨设备删除墓碑机制，防止设备2登录后用本地旧事项把云端删除结果顶回去
- 完成本地校验、提交推送与线上强刷复测

## 当前待办
- 🔄 待验证同账号双设备场景：设备 1 删除后，设备 2 本地旧事项不会“删除成功下一秒又回来”
- 🔄 待验证设备 2 新建事项删除后不会被登录同步立即复活
- 待完成本地语法检查、diagnostics、git 提交推送与线上强刷复测

## 已完成（本轮 v4.65）
- ✅ 已进一步定位真实根因：仅靠 items 对账无法表达“这个事项已经被删除”，所以设备 2 登录后仍会把本地旧事项重新参与合并并上传，导致删除成功后下一秒回流
- ✅ 已在 `sync.js` 增加删除墓碑 `deletedItemsMap` 并同步到云端 `deletedItems` 字段
- ✅ 已让 `buildReconciledItems()`、`syncLocalItemsToState()` 在合并与落地前优先尊重删除墓碑，已删除事项不再被另一设备本地旧记录复活
- ✅ 已在 `app.js` 的单条删除、批量删除、AI 删除路径中补上 `syncManager.markItemDeleted(...)`
- ✅ 已在撤回删除 / 重做新增 / 重做删除路径中补上删除墓碑清理与重建，避免撤回功能与同步机制互相打架
- ✅ 已将部署版本提升为 `2026-04-29 v4.65`，资源版本提升到 `sync.js?v=45`、`app.js?v=145`
