...
- ✅ 已保留 P2-1 第一轮的主面板按日期取数收敛成果，未回退既有优化
- ✅ 已完成周/月视图快速新增链路：点击空白单元格后可直接选择新增待办、会议或办文
- ✅ 已扩展 `showAddModal(type, selectedDate)`，支持按点击日期预填待办截止时间、会议日期、办文开始日期
- ✅ 已保留已有事项点击后跳转到对应日视图，不影响原有“查看当天事项”路径
- ✅ 已补强 `_createChoiceModal`，避免快速连续点击造成选择弹窗叠加
- ✅ 已完成本地脚本校验、诊断校验与页面级真人模拟测试
- 🔄 待提交、推送、部署本轮改动

### 本轮关键改动
- app.js：新增 `calendarQuickAdd` 事件监听，统一承接周/月视图空白点击后的快速新增入口
- app.js：`showAddModal` 新增 `selectedDate` 参数，使新增待办、会议、办文时能按点击日期自动预填默认值
- app.js：新增 `handleCalendarQuickAdd` / `showCalendarQuickAddTypeChoice`，让周/月视图新增前先选择事项类型
- app.js：增强 `_createChoiceModal`，同一时间只保留一个选择弹窗，并支持点击遮罩取消
- calendar.js：新增 `quickAddForDate`，周/月视图空白单元格点击改为触发快速新增事件
- calendar.js：周/月视图空白态统一显示“+ 点击新增”，已有事项卡片继续保留点击后跳到对应日视图
- css/style.css：新增 `calendar-empty-hint` 样式，统一周/月视图空白提示外观
- index.html：资源版本号更新为 `calendar.js?v=15`、`app.js?v=78`

### 验证结果
- `node --check js/app.js` 通过
- `node --check js/calendar.js` 通过
- app.js / calendar.js 诊断均为 0 错误
- 页面级真人模拟测试通过：
  - 周视图空白单元格点击 → 弹出类型选择 → 选择“待办事项”后打开新增弹窗，默认带入点击日期
  - 月视图当月空白单元格点击 → 弹出类型选择 → 选择“会议活动”后打开新增弹窗，默认带入点击日期
  - 已有事项卡片点击仍保留跳到对应日视图的既有行为
- Chrome Console 0 error / 0 warn
- 未改动数据库结构、未改动同步协议、未改动跨日期办文作用范围逻辑

### 遗留事项
- 本轮已完成用户提出的 P2 + 周/月视图空白新增需求
- 若后续继续做性能优化，可再单独评估 IndexedDB 日期索引或轻量缓存层
