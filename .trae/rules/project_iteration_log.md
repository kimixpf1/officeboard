...
- 修复跨日期办文在“修改今天及之后”后，卡片显示正确但点进详情/编辑仍显示未来内容的问题
...
### 本轮关键改动
- app.js：新增跨日期办文按选中日期解析的方法，统一 dayStates 覆盖逻辑
- app.js：列表渲染改为复用统一解析方法
- app.js：点击卡片标题/编辑按钮后，编辑回填前先按 selectedDate 解析办文内容
- app.js：流转入口改为复用按日解析结果，并对跨日期办文提供“仅当天 / 今天及之后 / 全部日期”三类作用范围
- app.js：办文保存时补齐当前选中日期下的流转历史回填，避免历史日期编辑覆盖错误记录
...
- app.js / project_framework.md / project_rules.md / project_iteration_log.md / todolist.md 诊断均为 0 错误
- index.html 已将 app.js 资源版本号提升到 `v75`
- 已补做流转入口复查，当前已确认列表渲染、卡片展开、编辑回填、流转提交共用按日解析逻辑
- 本轮补充修复已准备提交推送，待用户继续做线上复测，确认是否还存在其他未覆盖入口
